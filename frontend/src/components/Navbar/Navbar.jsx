        ...
        {user ? (
          <div className="flex items-center gap-4 sm:gap-5">
            <button onClick={() => navigate("/buy")} className="flex items-center gap-2 px-4 bg-blue-100 sm:px-6 py-1.5 sm:py-3 rounded-full hover:scale-105 transition-all duration-700">
                ...
                <p className="text-xs font-medium sm:text-sm">Credits left : {credit}</p>
            </button>
            
            <p className="pl-4 max-sm:hidden">Hi, {user.name}</p>
          ...
          <div className="flex items-center gap-2 text-lg font-medium sm:gap-5">
            ...
            <button onClick={() => setShowLogin(true)} className="py-2 text-white rounded-full bg-zinc-800 px-7 sm:px-10">
              Login
            </button>
          </div>
        )}
